...
DWORD CCompressor::EncodeToFile(BYTE* pBuffer, const DWORD BufferSize, FILE* OutF)
{
	CHuffmanCoder HuffmanCoder;
	return HuffmanCoder.EncodeToFile(pBuffer, BufferSize, OutF);
}

DWORD CCompressor::DecodeFromFile(BYTE* pBuffer, const DWORD ExpectedBufferSize, FILE* InF)
{
	CHuffmanCoder HuffmanCoder;
	return HuffmanCoder.DecodeFromFile(pBuffer, ExpectedBufferSize, InF);
}

DWORD CCompressor::DecodeFromMemory(BYTE* pDstBuffer, BYTE *pSrcBuffer, DWORD dwUncompressedSize, DWORD dwCompressedSize, DWORD NumEncodedChar, BYTE *pBinCodes)
{
	CHuffmanCoder HuffmanCoder;
	return HuffmanCoder.DecodeFromMemory(pDstBuffer, pSrcBuffer, dwUncompressedSize, dwCompressedSize, NumEncodedChar, pBinCodes);
}
...
DWORD CCompressor::CHuffmanCoder::EncodeToFile(BYTE* pBuffer, const DWORD BufferSize, FILE* OutF)
{
	...
	for(DWORD Cpt = 0; Cpt < NumNodesCount; Cpt++){
		BYTE b = 0;
		UINT Freq = 0;
...
DWORD CCompressor::CHuffmanCoder::DecodeFromFile(BYTE* pBuffer, const DWORD ExpectedBufferSize, FILE* InF)
{
	...
	// Convert our stats in a Priority list used to build the tree...
	for(DWORD Cpt = 0; Cpt < Stats.GetNodeCount(); Cpt++){
		BYTE b = 0;
		UINT Freq = 0;
		Stats.GetValue((BYTE)Cpt, &b, &Freq);
...
DWORD CCompressor::CHuffmanCoder::DecodeFromMemory(BYTE* pDstBuffer, BYTE *pSrcBuffer, DWORD dwUncompressedSize, DWORD dwCompressedSize, DWORD NumEncodedChar, BYTE *pBinCodes)
{
	////////////////////////////////////////////////////////////////////////////////////
	// Read back our bincode structure and build the stats table
	////////////////////////////////////////////////////////////////////////////////////
	...
	UINT Freq = 0;
	for(DWORD Cpt = 0; Cpt < NumEncodedChar; Cpt++){
		memcpy(&ch,        &pBinCodes[Indx], sizeof(BYTE));
		Indx += sizeof(BYTE);
		memcpy(&Freq,      &pBinCodes[Indx], sizeof(DWORD));
		Indx += sizeof(DWORD);
		memcpy(&BinCode,   &pBinCodes[Indx], sizeof(WORD));
		Indx += sizeof(WORD);
		memcpy(&BitsCount, &pBinCodes[Indx], sizeof(BYTE));
		Indx += sizeof(BYTE);
	...
	CBitStream BitStream;
	BitStream.Allocate(dwCompressedSize);
	BitStream.Fill(pSrcBuffer);
	
	for(DWORD Cpt = 0; Cpt < dwUncompressedSize; Cpt++)
		pDstBuffer[Cpt] = BitStream.DecodeNextChar(PriorityQueue.GetTreeRootNode());
...
void CCompressor::CStats::InitNode(StatsStruct *pNode, StatsStruct *pParentNode, BYTE ch)
{
	pNode->Char = ch;
	pNode->Frequency = 0;
...
void CCompressor::CStats::Initialize()
{
	KillNodes();
	InitNode(&RootNode, NULL, 0);
	...
	for(int Cpt = 0; Cpt < 256; Cpt++)
		pNode = AddChildNode(pNode, (BYTE)Cpt);
}

void CCompressor::CStats::KillNodes()
{
	StatsStruct *pNode;
	do {
		pNode = GetLastNode();
...
CCompressor::StatsStruct* CCompressor::CStats::GetLastNode()
{
	StatsStruct *pNode = &RootNode;
...
CCompressor::StatsStruct* CCompressor::CStats::AddChildNode(StatsStruct *pNode, BYTE ch)
{
...
bool CCompressor::CStats::RemoveChildNode(StatsStruct *pNode)
{
...
void CCompressor::CStats::ExchangeNodes(StatsStruct *pNode1, StatsStruct *pNode2)
{
...
UINT CCompressor::CStats::SortStats()
{
...
void CCompressor::CStats::RemoveEmptyStats()
{
...
void CCompressor::CStats::IncFrequency(BYTE Indx, DWORD Value)
{
...
void CCompressor::CStats::GetValue(BYTE Indx, BYTE *pChar, UINT *pFrequency)
{
	StatsStruct *pNode = RootNode.ChildNode;
	if(pNode == NULL){return;}
...
UINT CCompressor::CStats::GetNodeCount()
{
	UINT Count = 0;
	StatsStruct *pNode = &RootNode;
...
void CCompressor::CStats::WriteStats(char *fname)
{
...
CCompressor::CPriorityQueue::CPriorityQueue()
{
...
UINT CCompressor::CPriorityQueue::GetNodeCount()
{
	UINT Count = 0;
...
CCompressor::PriorityQueueStruct* CCompressor::CPriorityQueue::GetLastNode()
{
	PriorityQueueStruct *pNode = &RootNode;
...
void CCompressor::CPriorityQueue::AddLastNode(UINT Priority, BYTE ch)
{
...
CCompressor::PriorityQueueStruct* CCompressor::CPriorityQueue::InsertNode(UINT Priority, BYTE ch)
{
...
bool CCompressor::CPriorityQueue::RemoveFirstNode()
{
...
void CCompressor::CPriorityQueue::ClearNodes()
{
...
void CCompressor::CPriorityQueue::CombineTwoLowestFreqNodes()
{
	...
	pTreeNode->Char       = 0;
	pTreeNode->NodeType   = GetNodeCount() > 2 ? COMPRESSOR_NORMAL_NODE : COMPRESSOR_ROOT_NODE;
...
// Build our huffman tree... finally...
void CCompressor::CPriorityQueue::BuildTree()
{
	...
	while(pNode->ChildNode != NULL){
		...
		pTreeBase->NodeDat.NodeType   = COMPRESSOR_LEAF_NODE;
...
CCompressor::TreeStruct* CCompressor::CPriorityQueue::GetTreeRootNode()
{
...
void CCompressor::CPriorityQueue::KillTree()
{
...
void CCompressor::CPriorityQueue::KillTreeNode(TreeStruct *pNode)
{
	if(pNode == NULL)
		return;
...
// Return a pointer to the first base node in the tree...
CCompressor::TreeStruct* CCompressor::CPriorityQueue::GetBaseTreeNode(BYTE Indx)
{
	TreeBaseNodeListStruct *pBaseNode = TreeLeafsNode.Child;
...
DWORD CCompressor::CPriorityQueue::GenBinCodes(BinCodeStruct *pBinCodes)
{	
	DWORD NumEncodedChar = 0;
	...
	TreeStruct *pTreeNode = GetBaseTreeNode((BYTE)BaseNodeIndx);
	...
	while(pTreeNode != NULL){
		...
		while(pTreeNode != NULL){
			...
			if(pTreeNode->NodeType != COMPRESSOR_ROOT_NODE && pTreeNode->BinaryCode != 0){
				if(BitIndx <= 0)
		...
		BaseNodeIndx++;
		pTreeNode = GetBaseTreeNode((BYTE)BaseNodeIndx);
		if(BaseNodeIndx >= 256)
			break;
	}
...
CCompressor::CBitStream::CBitStream()
{
	pBuffer = NULL;
	IsAllocated = false;
...
void CCompressor::CBitStream::Allocate(DWORD Size)
{
	if(IsAllocated)
		this->Free();
...
void CCompressor::CBitStream::Erase()
{
	if(IsAllocated){
...
void CCompressor::CBitStream::Fill(BYTE* pBuf)
{
	if(IsAllocated){
		Erase();
...
void CCompressor::CBitStream::Free()
{
	if(IsAllocated){
...
void CCompressor::CBitStream::UpdateCounters(BYTE BitsProcessed)
{
...
void CCompressor::CBitStream::Save(FILE *OutF)
{
...
void CCompressor::CBitStream::EncodeNextChar(WORD wBinCode, BYTE bBitsCount)
{
...
BYTE CCompressor::CBitStream::DecodeNextChar(TreeStruct *pRootNode)
{
	...
	} while(pNode->NodeType != COMPRESSOR_LEAF_NODE);
